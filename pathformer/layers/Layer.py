...
from pathformer.layers.Embedding import *
...
class Transformer_Layer(nn.Module):
    def __init__(self, device, d_model, d_ff, num_nodes, patch_nums, patch_size, dynamic, factorized, layer_number, batch_norm,
        ...
        # Dual-attention ablation switches (Experiment A6). Default True/True
        # reproduces the original PathFormer behavior exactly.
    ...
    def forward(self, x):
        ...
        new_x  = x
        batch_size = x.size(0)
        b, seq_len, nvar, _ = x.shape
        # patch_nums is computed upstream as int(seq_len/patch_size), so when
        # patch_size does not evenly divide seq_len, intra/inter attention
        # only "see" the first eff_len=patch_nums*patch_size steps. We pad
        # their outputs with zeros back to the full seq_len so every expert
        # (any patch size) yields identical shapes for the residual add and
        # for SparseDispatcher.combine() across experts of different patch
        # sizes. No-op (pad amount 0) whenever patch_size evenly divides seq_len.
        eff_len = self.patch_nums * self.patch_size
        pad_len = seq_len - eff_len
        ...
        if self.use_intra:
            ...
            intra_out_concat = intra_out_concat.permute(0,3,2,1)
            if pad_len > 0:
                intra_out_concat = F.pad(intra_out_concat, (0, 0, 0, 0, 0, pad_len))
        else:
            intra_out_concat = torch.zeros_like(new_x)
        ...
        if self.use_inter:
            ...
            if pad_len > 0:
                inter_out = F.pad(inter_out, (0, 0, 0, 0, 0, pad_len))
        else:
            inter_out = torch.zeros_like(new_x)

        out = new_x + intra_out_concat + inter_out
        if self.batch_norm:
            out = self.norm_attn(out.reshape(b*nvar, seq_len, self.d_model))
        ##FFN
        out = self.dropout(out)
        out = self.ff(out) + out
        if self.batch_norm:
            out = self.norm_ffn(out).reshape(b, seq_len, nvar, self.d_model)
